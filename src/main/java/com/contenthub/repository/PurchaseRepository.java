...
public interface PurchaseRepository extends JpaRepository<Purchase, Long> {
    ...
    List<Purchase> findByUser_IdAndRazorpayOrderId(Long userId, String razorpayOrderId);

    List<Purchase> findByUser_IdAndStatusOrderByCreatedAtDesc(Long userId, Purchase.Status status);

    List<Purchase> findBySeller_IdAndStatus(Long sellerId, Purchase.Status status);
    ...
    @Query("select p.content.id from Purchase p where p.user.id = :userId and p.status = :status")
    List<Long> findPurchasedContentIds(@Param("userId") Long userId, @Param("status") Purchase.Status status);
